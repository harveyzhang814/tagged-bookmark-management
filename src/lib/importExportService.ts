...
import { getBookmarksMap, getTagsMap, saveBookmarksMap, saveTagsMap } from './storage';
import type {
  BookmarkItem,
  ImportExportFile,
...
export const importData = async (
  ...
  const result: ImportResult = {
    imported: { bookmarks: 0, tags: 0 },
    skipped: { bookmarks: 0, tags: 0 }
  };
  ...
  if (mode === 'overwrite') {
    ...
    Object.entries(fileData.data.bookmarks).forEach(([id, bookmark]) => {
      ...
      let finalId = id;
      if (newBookmarks[id]) {
        finalId = generateId('bm');
      }

      const clickHistory = includeHistory && bookmark.clickHistory ? bookmark.clickHistory : [];
      newBookmarks[finalId] = {
        ...bookmark,
        id: finalId,
        clickCount: clickHistory.length,
        clickHistory: clickHistory.length > 0 ? clickHistory : [],
