...
- 任意网页快捷键唤出的悬浮搜索层运行在 content 脚本中，无法使用 `react-i18next`，故采用 Chrome 原生 **chrome.i18n**（`_locales`）提供占位符、区块标题、无结果/错误提示等文案。
- 文案文件：`public/_locales/en/messages.json`、`public/_locales/zh_CN/messages.json`，key 如 `globalSearchPlaceholder`、`globalSearchNoResults`、`bookmarkTitle`、`tagTitle` 等；manifest 中设置 `default_locale: 'en'`。
