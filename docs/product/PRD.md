...
- 关联浏览
  ...
  - 添加书签到工作区弹窗（AddBookmarkToWorkstationModal）：居中悬浮；搜索框匹配书签标题/URL/标签名；默认展示置顶书签、有关键词时展示搜索结果；选中区展示「本次已选中」书签，与结果区共用加号按钮（添加/选中态切换）；添加或移除立即写入工作区关系；关闭弹窗时刷新页面数据（工作区卡片与侧栏列表）
